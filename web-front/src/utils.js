...
export const check_TokensEnable = (that) => {
  ...
  that.state.XSwap_stable.methods.tokensEnable(address_map[that.state.net_type]['GOLDx']).call().then(res_TokensEnable => {
    // console.log(res_TokensEnable);
    that.setState({
      is_tokensEnable_GOLDx: res_TokensEnable
    })
  })
...
export const handle_A_max = (that) => {
  ...
  } else if (that.state.cur_send_addr === 'GOLDx') {
